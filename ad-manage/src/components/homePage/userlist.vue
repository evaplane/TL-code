    ...
    <div class='userContent'>
        ...
        <div class='content' ref="content">
            <el-row class='operation'>
                <el-col :span="24">
                    <el-radio-group v-model="tabPosition" style="margin-bottom: 10px;margin-left:30px;" @change="handleRadioChange">
                        <el-radio-button label="新增数">新增用户</el-radio-button>
                        <el-radio-button label="活跃数">活跃用户</el-radio-button>
                    </el-radio-group>
            ...
            <el-dialog
                ...
                <div ref="usersHours" :style="{height: '500px',margin: '0px 10px',padding:'10px 0 0 5px'}"></div>
                </el-dialog>
        </div>
...
export default {
    ...
    created(){
        ...
        if(!slice){
            ...
            slice = 1;
            this.tabPosition = "新增数";
        }else if(slice == 1){
            this.tabPosition = "新增数";
        }else if (slice == 2){
            this.tabPosition = '活跃数';
        };
        this.allList();
    },
    mounted(){
    ...
    methods: {
        allList(){
            this.$get('common-web/point/userTotalList').then((res)=>{
                if(res.code == '000000'){
                    res.data.forEach(v=>{
                        v.todayAliveCount = String(v.todayAliveCount)+'('+(v.total <= 0 ? "0%" : (Math.round(v.todayAliveCount / v.total * 10000) / 100.00)+"%")+')';
                    })
                    this.tableData = res.data;
                    this.firstGetVersion();
                }else{
        ...
        reloadData(){
            let url;
            if(this.tabPosition == '新增数'){
                url = 'common-web/point/newUserDayList';
            }else{
                url = 'common-web/point/aliveUserDayList';
            };
            let data = {};
        ...
        handleOptions(allData){
            ...
            for (let i = 0; i < allData.length; i++) {
                ...
                options.legend.data.push(list.version);
            ...
            for (let i = 0; i < allData.length; i++) {
                ...
                switch(list.platform){
                    ...
                    case 3:
                        ...
                        break;
                    default:
                        obj4.name = list.version;
                        legendData = [list.version];
        ...
        loadHoursData(version,dayTime,offsetX,offsetY,color){
            let url;
            if(this.tabPosition == '新增数'){
                url = 'common-web/point/newUserHourList';
            }else{
                url = 'common-web/point/aliveUserHourList';
            };
        ...
        onClickbind(){
            ...
            this.usersDayEcharts.on('click', (handler) => {
                if(new Date(handler.name).getTime() < new Date().getTime()){
                    this.dialogTitle = handler.name + ' ' + this.tabPosition;
