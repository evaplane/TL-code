<template>
    <div class='advContent'>
        ...
        <el-col :span="4">
            ...
            <el-form-item label=" ">
                <el-input placeholder="广告名称" v-model="dayData.advertName" clearable></el-input>
            </el-form-item>
            </el-form>
        </el-col>
        <el-col :span="4">
            <el-form :inline="true" class="demo-form-inline">
            <el-form-item label="">
                <el-input placeholder="广告分类名称" v-model="dayData.categoryName" clearable></el-input>
            </el-form-item>
            </el-form>
        </el-col>
        <el-col :span="6">
        ...
        <el-col :span="2">
            <el-button type="primary" size="medium"  @click='reloadData'>查询</el-button>
        </el-col>
        </el-row>
        <!-- 内容 -->
        <div class='content' ref="content">
            <el-row class='operation'>
                <el-col :span="24">
                    <div v-show="showData" ref="usersDay" :style="{height: '500px',margin: '0px 10px',padding:'10px 0 0 5px'}"></div>
            ...
            <el-dialog
                ...
                <div v-show="showDialogData" ref="usersHours" :style="{height: '500px',margin: '0px 10px',padding:'10px 0 0 5px'}"></div>
                <div v-show="!showDialogData" :style="{height: '500px',lineHeight:'500px',textAlign:'center'}">暂无数据</div>
                </el-dialog>
        </div>
...
export default {
    ...
    methods: {
        allList(){
            this.$get('common-web/point/advertTotalList').then((res)=>{
                if(res.code == '000000'){
        ...
        reloadData(){
            ...
            // 绘制图表
            this.$get('common-web/point/advertDayList',data).then(res => {
                if (res.code == '000000') {
        ...
        handleOptions(allData){
            ...
            for (let i = 0; i < allData.length; i++) {
                ...
                let obj = {
                    type:'line',
                    name:list.advtName,
                    uname:list.categoryName,
                    data:[]
                };
                console.log(list.categoryName + ' ' + list.advtName)
                options.legend.data.push(list.advtName);
            ...
            // this.editionTypeData = editionTypeData;
            return options;
        },
            ...
            for (let i = 0; i < allData.length; i++) {
                ...
                switch(list.platform){
                    ...
                    case 3:
                        ...
                        break;
                    default:
                        obj4.name = list.categoryName;
                        legendData = [list.categoryName];
        ...
        loadHoursData(advtName,dayTime,offsetX,offsetY,color){
            var data  = this.chartOptions.series.filter(function (item) {
                return item.name == advtName;
            });
            let url;
            dayTime = dayTime.replace(/-/g,'/');
            dayTime += ' 00:00:00';
            // 绘制图表
            this.$get('common-web/point/advertHourList',{
                dayTime:dayTime,
                platform:'',
                categoryName:data[0].unanme,
                advertName:data[0].name
            }).then(res => {
                if (res.code == '000000') {
        ...
        handleOpen(){
            this.usersHoursEcharts = this.$echarts.init(this.$refs['usersHours']);
            this.usersHoursEcharts.setOption(this.chartOptionsHours);
            console.log(this.chartOptionsHours)
        },
        handleClose(done){
            console.log(1);
            done();
