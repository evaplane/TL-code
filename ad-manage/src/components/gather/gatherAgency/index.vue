  ...
  <div class='user'>
    ...
    <el-row class='operation'>
      <el-col :span="8" style="padding-bottom:20px">
        <!-- 按钮 -->
        <el-button type="primary" v-if="this.$store.state.roleList.indexOf('collectProxyPool:save') > -1" size="medium"  @click="addGatherAyencyVisible=true">新增</el-button>
        <el-button type="primary" v-if="this.$store.state.roleList.indexOf('collectProxyPool:update') > -1"  size="medium" @click="handleEdit">修改</el-button>
        <el-button type="primary" v-if="this.$store.state.roleList.indexOf('collectProxyPool:delete') > -1"  size="medium" @click="handleDelete">删除</el-button>
      </el-col>
    </el-row>
    <!-- 内容 -->
    <div class='content'>
        <!-- 表格 -->
      <el-table ref="multipleTable" border :data="gatherUserTableData" tooltip-effect="dark" style="width: 100%"  @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="proxyIp" label="IP" width="150" :resizable = "false"></el-table-column>
        <el-table-column prop="proxyPort" label="端口号" width="150" :resizable = "false"></el-table-column>
        <el-table-column prop="status" label="状态" width="100" :resizable = "false">  
          <template slot-scope="scope">
            ...
            <switchItem :val="scope.row.status" :disabled="statusDisabled" :id="scope.row.id" :reqUrl="'common-web/collectProxyPool/updateStatus'" @result="getGatherAgencyData"></switchItem>
          </template>
        </el-table-column>
        <el-table-column prop="collectTime" label="采集时间" width="150" :resizable = "false"></el-table-column>
      ...
      <!-- 新增框 -->
      <el-dialog title="新增" :visible.sync="addGatherAyencyVisible" class='gatherDialog' :before-close="handleClose">
        ...
        <el-form :model="addGatherAyencyForm" label-width="100px" style='margin-top:10px;' :rules="addGatherAyencyRules" ref="addGatherAyencyForm">
          <el-form-item label="用户昵称" prop="userName">
            <el-input v-model="addGatherAyencyForm.userName" autocomplete="off" placeholder="用户昵称"></el-input>
          </el-form-item>
          <el-form-item label="IP" prop="proxyIp">
             <el-input v-model="addGatherAyencyForm.proxyIp" autocomplete="off" placeholder="IP"></el-input>
          </el-form-item>
          <el-form-item label="端口号" prop="proxyPort">
             <el-input v-model="addGatherAyencyForm.proxyPort" autocomplete="off" placeholder="端口号"></el-input>
          </el-form-item>
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="addGatherAyencySubmit('addGatherAyencyForm')">确 定</el-button>
        </div>
      </el-dialog>
      <!-- 修改框 -->
      <el-dialog title="修改" :visible.sync="editGatherAyencyVisbile" class='gatherDialog'>
        <div style="width:100%;height:100%;position:absolute;z-index:1000;top:0;left:0;" 
          :style="{width:loading?'100%':'1px',height:loading?'100%':'1px'}" 
          v-loading="loading2"
          element-loading-text="正在请求中"
        ...
        <el-form :model="editGatherAyencyForm" label-width="100px" style='margin-top:10px;' :rules="editGatherAyencyRules" ref="editGatherAyencyForm">
          <el-form-item label="用户昵称" prop="userName">
            <el-input v-model="editGatherAyencyForm.userName" autocomplete="off" placeholder="用户昵称"></el-input>
          </el-form-item>
          <el-form-item label="IP" prop="proxyIp">
             <el-input v-model="editGatherAyencyForm.proxyIp" autocomplete="off" placeholder="IP"></el-input>
          </el-form-item>
          <el-form-item label="端口号" prop="proxyPort">
             <el-input v-model="editGatherAyencyForm.proxyPort" autocomplete="off" placeholder="端口号"></el-input>
          </el-form-item>
          <el-form-item label="采集时间" prop="collectTime">
            <el-row type="flex" justify="space-between">
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="editGatherAyencyVisbile = false">取 消</el-button>
          <el-button type="primary" @click="editGatherAyencySubmit('editGatherAyencyForm')">确 定</el-button>
        </div>
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  data() {
     ...
     var proxyPort = (rule, value, callback) => {
        if (value === '') {
          callback(new Error('请输入端口号'));
        } else { 
          var reg = /^([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-4]\d{4}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/
          if (!reg.test(value)) {
            callback(new Error('请输入0-65535数字'));
          }
          callback();
        }
     };
     var proxyIp = (rule, value, callback) => {
         if (value === '') {
          ...
          if (!reg.test(value)) {
            callback(new Error('请输入正确格式,例:127.0.0.1'));
          }
          callback();
        }
     };
    ...
    return {
      ...
      // 修改框数据
      editGatherAyencyForm:{
        userName:'',
        proxyIp:'',
        proxyPort:''
      },
      // 显示重置密码框
      addGatherAyencyVisible:false,
      // 新增规则
      addGatherAyencyRules:{
        userName: [
          {required:true,trigger: "change",validator: userName}
        ],
        proxyIp: [
          {required:true,trigger: "change",validator: proxyIp}
        ],
        proxyPort: [
          {required:true,trigger: "change",validator: proxyPort}
        ],
        collectTime: [
  ...
  created() {
    this.statusDisabled = this.$store.state.roleList.indexOf('collectProxyPool:update') == -1;
    const query = this.$route.query;
    this.userData.pageNo = Number(query.page)||1;
    this.userData.pageSize = Number(query.pageSize)||10;
    this.getGatherAgencyData();
  },
  computed:{
    addSelectableRange(){
  ...
  methods: {
    ...
    // 获取用户
    getGatherAgencyData() {
      this.$post('common-web/collectProxyPool/pageList', this.userData).then(res => {
        if (res.code == '000000') {
    ...
    handleEdit(){
      if(this.selectRows.length==0){
        ...
        this.editGatherAyencyVisbile=true;
        this.editGatherAyencyForm.userName = this.selectRows[0].userName
        this.editGatherAyencyForm.proxyIp = this.selectRows[0].proxyIp
        this.editGatherAyencyForm.proxyPort = this.selectRows[0].proxyPort
    ...
    statechange(row){
      ...
      this.$post('common-web/collectProxyPool/updateStatus',data).then(res=>{
        if(res.code=='000000'){
    ...
    handleDelete(){
      if(this.selectRows.length==0){
        ...
        this.$confirm('是否确认删除?', '提示', {
          ...
          ids = ids.join(',')
          this.$post(`common-web/collectProxyPool/delete`,{ids:ids}).then(res=>{
          if(res.code=='000000'){
              this.$message({
                type: 'success',
                message: '删除成功!'
    ...
    // 新增提交
    addGatherAyencySubmit(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          let data ={
            userName:this.addGatherAyencyForm.userName,
            proxyIp:this.addGatherAyencyForm.proxyIp,
            proxyPort:this.addGatherAyencyForm.proxyPort,
            collectTimeType:collectTimeType,
            collectTimeStart:addStartTime,
            collectTimeEnd:addEndTime
          }
          this.$post('common-web/collectProxyPool/save',data).then(res=>{
            this.loading = false;
            if(res.code=='000000'){
              this.$message.success(res.message)
              this.handleClose()
              this.addGatherAyencyVisible = false;
              this.getGatherAgencyData();
            }else{
              this.$message.warning(res.message)
            }
    ...
    // 修改提交
    editGatherAyencySubmit(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading2 = true;
          let editStartTime ="0"
          ...
          let data ={
            userName:this.editGatherAyencyForm.userName,
            proxyIp:this.editGatherAyencyForm.proxyIp,
            proxyPort:this.editGatherAyencyForm.proxyPort,
            collectTimeType:collectTimeType,
            collectTimeStart:editStartTime,
            collectTimeEnd:editEndTime,
            id:this.selectRows[0].id
          }
          this.$post('common-web/collectProxyPool/update',data).then(res=>{
            this.loading2 = false;
            if(res.code=='000000'){
              this.$message.success(res.message)
              this.editGatherAyencyVisbile = false;
              this.getGatherAgencyData();
            }else{
              this.$message.warning(res.message)
            }
    ...
    // 关闭
    handleClose(){
      this.$refs.addGatherAyencyForm.clearValidate()
      this.$refs['addGatherAyencyForm'].resetFields()
      this.allDay=false;
      document.getElementsByClassName('startTime')[0].children[0].value=''
      document.getElementsByClassName('endTime')[0].children[0].value=''
      this.addGatherAyencyVisible=false;
