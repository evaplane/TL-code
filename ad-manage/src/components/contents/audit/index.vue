      ...
      <el-col :span="8">
        <el-form :inline="true" class="demo-form-inline">
          <el-form-item label="审核不通过原因:">
            <el-input placeholder="请输入审核不通过原因" clearable v-model="labelData.reasonDesc" type="text"></el-input>
          </el-form-item>
        </el-form>
      </el-col>
      <el-col :span="8">
        <!-- 按钮 -->
        <el-button type="primary" v-if="this.$store.state.roleList.indexOf('auditRejectReason:pageList') > -1" size="medium" @click='handleSearch'>查询</el-button>
        <el-button type="primary" v-if="this.$store.state.roleList.indexOf('auditRejectReason:save') > -1" size="medium" @click="handleAdd">新增</el-button>
        <el-button type="primary" v-if="this.$store.state.roleList.indexOf('auditRejectReason:update') > -1" size="medium" @click="handleEdit">修改</el-button>
        <el-button type="primary" v-if="this.$store.state.roleList.indexOf('auditRejectReason:delete') > -1" size="medium" @click="handleDelete">删除</el-button>
      </el-col>
    </el-row>
    <!-- 内容 -->
    <div class='content'>
      ...
      >
        ...
        <el-table-column prop="reasonDesc" label="审核不通过原因"  show-overflow-tooltip :resizable = "false"></el-table-column>
      ...
      <el-dialog :title="addDeptForm.title" :visible.sync="addDeptForm.adCategoryAddVisible" :before-close="closeDialog">
        ...
        <el-form :model="addDeptForm" :rules="addDeptRules" ref="addDeptForm" label-width="130px" style="margin-top: 10px; margin-left: 40px;" class="demo-ruleForm">
          <el-form-item label="审核不通过原因: " prop="reasonDesc">
            <el-input type="textarea" :rows="5" v-model="addDeptForm.reasonDesc" autocomplete="off" placeholder="审核不通过原因" ></el-input>
          </el-form-item>
        </el-form>
...
export default {
  ...
  data() {
    var validatorDesc = (rule, value, callback) => {
        if (value === '') {
          ...
          var reg = /^[A-Za-z0-9\u4e00-\u9fa5\u3002\uff1b\uff0c\uff1a\u201c\u201d\uff08\uff09\u3001\uff1f\u300a\u300b]{1,64}$/
          if (!reg.test(value)) {
            callback(new Error('只能输入1-64位数字、字母或者中文'));
          }
          callback();
        }
    };
  ...
  created() {
    ...
    this.labelData.reasonDesc = query.reasonDesc;
    this.getDeptData();
  },
  methods: {
    // 获取列表数据
    getDeptData(flag) {
      ...
      query.reasonDesc = this.labelData.reasonDesc;
      this.$router.replace({query:query});
      this.$post("common-web/auditRejectReason/pageList",{
        reasonDesc:this.labelData.reasonDesc,
    ...
    handleEdit(){
      ...
      if(Object.keys(this.selectEditRow).length==0){
        ...
        this.addDeptForm.reasonDesc = this.selectEditRow[0].reasonDesc;
        this.addDeptForm.adCategoryAddVisible = true;
      }
    },
    // 新增修改弹窗确定按钮
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if(this.addDeptForm.title === "新增"){
            this.createdHandle({
              reasonDesc:this.addDeptForm.reasonDesc
            });
          }else{
            this.editHandle({
    ...
    // 新增
    createdHandle(data){
      this.$post("common-web/auditRejectReason/save",data).then(res=>{
        this.loading = false;
        if(res.code=='000000'){
          this.closeDialog();
          this.$message({
    ...
    // 修改
    editHandle(data){
      this.$post("common-web/auditRejectReason/update",data).then(res=>{
        this.loading = false;
        if(res.code=='000000'){
          this.closeDialog();
          this.$message({
      ...
      if(Object.keys(this.selectEditRow).length==0){
        ...
        return
         ...
         this.$confirm('是否确认删除?', '提示', {
          ...
          this.$post('common-web/auditRejectReason/delete',{ids: deleteArr.join(",")}).then(res=>{
            if(res.code=='000000'){
              this.$message({
                type: 'success',
                message: '删除成功!'
