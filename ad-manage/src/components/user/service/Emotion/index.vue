  ...
  <div>
    <div class="emotion-box" :style="{height: height + 'px' }" >
      <div class="emotion-box-line" v-for="(line, i) in list" :key="i" >
        <emotion class="emotion-item" v-for="(item, i) in line" :key="i" @click.native="clickHandler(item,i)" >{{item}}</emotion>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    clickHandler (i,index) {
      let emotion = `#${i};`
      this.$emit('emotion', emotion)
      this.$emit("qqShow", this.qqShow);
    }
  },
  components: {
    Emotion
