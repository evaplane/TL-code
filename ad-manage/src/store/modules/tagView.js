...
const tagsview ={
    ...
    mutations:{//这
        ADD_VISITED_VIEWS:(state,view)=>{//打开新页签--添加路由数据的方法      if(state.visitedviews.some(v=>v.path==view.path))return;
          // if(view.path.indexOf('Edit')!=-1 || view.path.indexOf('Detail')!=-1 ){
          //   return
          // }
        ...
        closeAllViews(state){
          state.visitedviews=[
            {name:'pageHome',path:'/modules/pageHome.html',title:'首页'}
          ]
        },
        closeOtherViews(state,current){
          console.log(state.visitedviews);
          for(let i=0;i<state.visitedviews.length;i++){
            if(current!=state.visitedviews[i].title){
              state.visitedviews.splice(i,1)
              i--
            }
          };
