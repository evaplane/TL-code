 ...
 */
export const otcOrderTimer = function (timeout: string | number | null, orderTime: null | string): any {
  if (!orderTime || !timeout) return;
  let nowDate = new Date();
  let nowDateTamp = nowDate.getTime();
  let orderTimeTamp = new Date(orderTime.replace(/(-)/g, '/')).getTime();
  let endDate = orderTimeTamp + (+timeout * 60 * 1000);
  let distance = endDate - nowDateTamp;
  let m, s, ms;
  let timeStr = '';
  if (distance >= 0) {
    ...
    timeStr = `${m}:${s} ${ms.toString().substring(0, 2)}`;
  } else {
    timeStr = '00:00';
  }
  return timeStr;
