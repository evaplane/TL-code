        ...
        render: function () {
            ...
            if (pages.first) {//如果是第1页
                ...
                if(pages.current==pages.first){
                	$(first).addClass('disabled').removeClass('active');
                	$(first).find('a').addClass('btn disabled').removeClass('active');
                }
            }
            ...
            if (pages.prev) {//如果是上一页
                ...
                if(pages.current==pages.first){
                	...
                	$(prev).find('a').addClass('btn disabled').removeClass('active');
                }
            }
            ...
            if (pages.next) {//如果是下一页
                ...
                if(pages.current==pages.total){
                	...
                	$(next).find('a').addClass('btn disabled').removeClass('active');
                }
            }
            ...
            if (pages.last) {//如果是最后一页
                ...
                //新加末页变灰
                if(pages.current==pages.total){
                	$(last).addClass('disabled').removeClass('active');
                	$(last).find('a').addClass('btn disabled').removeClass('active');
                }
            }
			//新加的
            var itemCustom = $("<li></li>");//创建一个容器
            var itemHtml = "<span><input type='text' style='width: 30px;height: 16px;' value='"+this.currentPage+"'/>/"+this.totalPages+"</span>";
...
//          if (page > this.totalPages || page < 0) {// 如果当前页码超出范围，则抛出异常
//
//              throw "页码超出范围";
//
//          }
    ...
    $.fn.bootstrapPaginator.defaults = {
        ...
        itemContainerClass: function (type, page, current) {
            return (page === current) ? "active" : "disabeled";
        },
