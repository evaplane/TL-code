...
function (t) {
    ...
    var i = {
        ...
        onClosed: null,
        icon_type: "class",
        template: '<div data-notify="container" class="col-xs-11 col-sm-4 alert alert-{0}" role="alert"><span data-notify="icon"></span><span class="notifyRight"> <span data-notify="title">{1}</span> <span data-notify="message">{2}</span></span><div class="progress" data-notify="progressbar"><div class="progress-bar progress-bar-{0}" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 0%;"></div></div><a href="{3}" target="{4}" data-notify="url"></a></div>'
    };
    ...
    t.extend(e.prototype, {
        ...
        bind: function () {
            var s = this;
            if (this.$ele.on("click", function () {
                var temp = s.settings.content.message.slice(4,6);
                console.log(temp);
				var dataTab ='';
				if(temp == "入款"){
                    var thisObj = JSON.parse(s.settings.content.url.replace(/#/g, ""));
					var params = {
						orderNo: thisObj.orderNo,
						operatorCode: _loginJsonObj.loginName,
					}
					
					sendAjax_notify('POST', host_url + "cloud-web/coreMessageNotify/financeMessageClick", params, function (res) {
						$(".account_children li").each(function(i,item){
							// 入金补单的跳转
							var localLevel = JSON.parse(
								window.localStorage.getItem("sybJsonLevel")
							).paramValue;
							if (
								parseFloat(localLevel).toFixed(1) == thisObj.acctLevel &&
								thisObj.notifyType == 10
							) {
								if($(item).text()=="入款补单" || $(item).text()=="入金补单"){
									dataTab = $(item).attr('data-tab');
									$("#tabContainer").data("tabs").$element.find(".nav-tabs li a[href='#"+dataTab+"']").click();
									$(".account_children li[data-tab='"+dataTab+"']").click();
									$("#corePayTradeFlowFormTable").bootstrapTable('refresh');
								}	
							}else{
								if($(item).text()=="入款作业" || $(item).text()=="入金作业"){
									dataTab = $(item).attr('data-tab');
									$("#tabContainer").data("tabs").$element.find(".nav-tabs li a[href='#"+dataTab+"']").click();
									$(".account_children li[data-tab='"+dataTab+"']").click();
									$("#corePayTradeFlowFormTable").bootstrapTable('refresh');
					...
					var params = {
						orderNo: thisObj.orderNo,
						routeCode: thisObj.routeCode,
						operatorCode: _loginJsonObj.loginName
					}
					sendAjax_notify('POST', host_url + "cloud-web/corePayTransFlow/financeGrabOrder", params, function (res) {
						$(".account_children li").each(function(i,item){
							if($(item).text()=="提款作业" || $(item).text()=="出金作业"){
								dataTab = $(item).attr('data-tab');
								$("#tabContainer").data("tabs").$element.find(".nav-tabs li a[href='#"+dataTab+"']").click();
								$(".account_children li[data-tab='"+dataTab+"']").click();
					...
					sendAjax_notify('POST', host_url + "cloud-web/coreMessageNotify/financeChangeMessageClick", params, function (res) {
						$(".account_children li").each(function(i,item){
							if($(item).text()=="提款作业" || $(item).text()=="出金作业"){
								dataTab = $(item).attr('data-tab');
								$("#tabContainer").data("tabs").$element.find(".nav-tabs li a[href='#"+dataTab+"']").click();
					...
					sendAjax_notify('POST', host_url + "cloud-web/coreMessageNotify/financeSplitMessageClick", params, function (res) {
						$(".account_children li").each(function(i,item){
							if($(item).text()=="提款作业" || $(item).text()=="出金作业"){
								dataTab = $(item).attr('data-tab');
								$("#tabContainer").data("tabs").$element.find(".nav-tabs li a[href='#"+dataTab+"']").click();
					...
					sendAjax_notify('POST', host_url + "cloud-web/coreMessageNotify/financeGroupMessageClick", params, function (res) {
						$(".account_children li").each(function(i,item){
							if($(item).text()=="入款作业复核" || $(item).text()=="入金作业复核"){
								dataTab = $(item).attr('data-tab');
								$("#tabContainer").data("tabs").$element.find(".nav-tabs li a[href='#"+dataTab+"']").click();
								$(".account_children li[data-tab='"+dataTab+"']").click();
					...
					sendAjax_notify('POST', host_url + "cloud-web/corePayMergeFlow/changeGrad", params, function (res) {
						$(".account_children li").each(function(i,item){
							if($(item).text()=="财务归集作业" || $(item).text()=="归集作业"){
								dataTab = $(item).attr('data-tab');
								$("#tabContainer").data("tabs").$element.find(".nav-tabs li a[href='#"+dataTab+"']").click();
								$(".account_children li[data-tab='"+dataTab+"']").click();
        ...
        close: function () {
            var s = this,
                i = !1,
                e = parseInt(this.$ele.css(this.settings.placement.from))*i;
