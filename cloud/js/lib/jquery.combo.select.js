...
(function (factory) {
	...
	var defaults = {			
		...
		comboClearAllClass : 'combo-clearAll',
	...
	$.extend(Plugin.prototype, {
		...
		_events: function(){
			...
			this.$container.on('mouseover', 'input', function(e){
				e.preventDefault()
			})
			...
			this.$container.on('click.arrow', '.'+this.settings.comboClearAllClass , function(){
				that.clearAll();
			})
		...
		_updateInput: function(){
			...
			if(this.$el.val()){
				if(this.$el.val().indexOf('+') ==-1){
					text = this.$el.find('option').eq(selected).text();
				}else{
					text = this.$el.find('option').eq(selected).attr('value');
				}

				this.$input.val(text)
				if(this.$el.find('option[value=""]').length > 0){
					this.$arrow[0].className = this.settings.comboClearAllClass;
				}

			}else{
				
				this.$input.val('')
				this.$arrow[0].className = this.settings.comboArrowClass;
			}
