...
(function ($, window, document, undefined) {
    ...
    var pluginName = 'tabs';
		window.tabLists = sessionStorage.getItem('tabLists') ? JSON.parse(sessionStorage.getItem('tabLists')) : {};
    ...
    BaseTab.prototype.template = {
		...
		div_content_panel_refresh: '<div class="tab-pane fade active in" id="{0}"></div>'
    }
...
//          console.error("showIndex超出了范围");
    ...
    BaseTab.prototype.builder = function (data) {
        var ul_div = $(this.template.ul_div);
        ...
        ul_div.append(ul_nav);

        this.$element.append(ul_div);
        $('.tabContent').append(div_content);
        this.loadData();
    ...
    BaseTab.prototype.addTab=function (obj) {
		...
		for(var i in tabLists){
			delete(tabLists[i].active)
		}
		if(tabLists[textid]){
			tabLists[textid].active = true;
		}else{
			tabLists[textid] = {};
			tabLists[textid].id = textid;
			tabLists[textid].text = obj.text;
			tabLists[textid].closeable = true;
			tabLists[textid].url = obj.url;
			tabLists[textid].active = true;
		}
        ...
        $('.tabContent').find(".tab-content:eq(0)").append(div_content_panel);
        ...
        if(obj.closeable){
            this.$element.find(".nav-tabs li a[href='#" + obj.id + "'] i.closeable").click(function () {
                ...
                if(self.getCurrentTabId()==href){
                    self.$element.find(".nav-tabs li:eq(0) a").tab("show");
                    $('.tabContent').find(".tab-pane:eq(0)").addClass('fade active in');
//                  this.$element.parents('.content-title').next('.tabContent').find(".tab-pane:eq(0)").addClass('fade active in');
                    $('.account_children li:eq(0)').addClass('active').siblings().removeClass('active');
                }
                $(this).parents("li").remove();
                $("#" + href).remove();
				delete tabLists[href];
				sessionStorage.setItem('tabLists',JSON.stringify(tabLists));
            })
        }
        //自定义添加代码
        var overWidth=0;
        $(".over ul li").each(function(){
        	overWidth+=$(this).width()+21.3;
        });
        var lengWidth=overWidth/($(".over ul li").length-0.5);
        var tabsLeft=$('#tabContainer .nav-tabs').css('left');
        
        $(".over ul").css({width:overWidth+"px"});
        if(overWidth>$(".over").width()){
        	$('#tabContainer .nav-tabs').css('left',parseInt(tabsLeft)-lengWidth);
        }
	...
	// 重新刷新页面
	BaseTab.prototype.refresh=function (obj) {
		var self=this;
		$("#" + obj.id).remove();
		var div_content_panel_refresh = $(this.template.div_content_panel_refresh.format(obj.id));
		$('.tabContent').find(".tab-content:eq(0)").append(div_content_panel_refresh);
		$("#" + obj.id).load(obj.url,obj.paramter);
		this.stateObj[obj.id] = true;
