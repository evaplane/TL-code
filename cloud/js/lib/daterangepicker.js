    ...
    DateRangePicker.prototype = {
        ...
        move: function () {
            ...
            if (this.opens == 'left') {
                ...
                } else if (containerLeft + this.container.outerWidth() > $(window).width()) {
                    this.container.css({
                        top: this.element.offset().top + this.element.outerHeight() - parentOffset.top,
                        left: 'auto',
                        right: 0
            ...
            if (cal.hasClass('left')) {
                ...
                if (typeof this.dateLimit === 'object') {
                    ...
                    if (endDate.isAfter(maxDate)) {
                        // 个性化设置 开始
                        var oldHour = endDate._d.getHours();
                        var oldMinutes = endDate._d.getMinutes();
                        var oldSeconds = endDate._d.getSeconds();
                        var oldTimestamp = (oldHour*3600+oldMinutes*60+oldSeconds)*1000;
                        endDate = maxDate;
                        var newTimestamp = endDate._d.getTime()+oldTimestamp;
                        endDate._d = new Date(newTimestamp)
                        // 个性化设置 结束
                ...
                endDate = this.rightCalendar.calendar[row][col];
                 // 个性化设置 开始
                 var oldSeconds = this.endDate._d.getSeconds();
                 var oldTimestamp = oldSeconds*1000;
                 var newTimestamp = endDate._d.getTime()+oldTimestamp;
                 endDate._d = new Date(newTimestamp)
                 // 个性化设置 结束
                if (typeof this.dateLimit === 'object') {
...
//          }
            ...
            if (isLeft) {
                ...
                this.leftCalendar.month.hour(hour).second(second);
            } else {
                ...
                this.rightCalendar.month.hour(hour).second(second);
            }
            this.updateCalendars();
        },
            ...
            if (this.timePicker) {
                ...
                for (i = 0; i < 60; i += this.timePickerIncrement) {
                    ...
                    if (i == selected.second()) {
                        html += '<option value="' + i + '" selected="selected">' + num1 + '</option>';
                    } else {
                        html += '<option value="' + i + '">' + num1 + '</option>';
                    }
                }
