 ...
 * Vue SSR要求我们建立entry-client.js和entry-server.js分别作为webpack的两端入口，两者各存在一份。这里entry-client.js的工作包括
 * 和服务端同步状态 (__INITIAL_STATE__)
 * 注册并调用中间件
 * 在客户端调用asyncData()为每个页面获取数据
 * 注册其他路由事件(如router.beforeEach等)
 */
...
if (!usingAppshell && ssr) {
  ...
  if (ssr || !enableAsyncCSS || (enableAsyncCSS && window.STYLE_READY)) {
    window.mountLavas();
  }
}
